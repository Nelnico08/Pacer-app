        ...
        <div className='flex flex-col items-center'>
          {equivalence && equivalence?.map(elem => (
            <div key={elem.distance} className='flex hover:bg-amber-400 cursor-default md:hover:scale-110 duration-500' >
              <p className='px-2 py-1 border border-orange-900 font-bold'>Distancia: {elem.distance < 10 ? `0${elem.distance}` : Number.isInteger(elem.distance) ? elem.distance : elem.distance === 21.1 ? 21 : 42}km</p>
              <p className='px-2 py-1 border border-orange-900 font-bold'>Tiempo: {elem.time.hour}:{elem.time.minutes}:{elem.time.seconds}</p>
              <p className='px-2 py-1 border border-orange-900 font-bold'>Ritmo: {elem.pace.minutes}:{elem.pace.seconds}min/km</p>
            </div>
          ))}
        </div>
      </div>
